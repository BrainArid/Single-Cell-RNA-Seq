...
We compared these output module sets at the module level and the gene level. A breakdown of these modules labeled with their most significant biological process ontology, cellular component ontology, and molecular function ontology is shown for the single-cell data in fig. \ref{fig:57872Clusts} and for the population data in fig. \ref{fig:48865Clusts}.

We then profiled the two lists of output ontology enriched modules to find both consensus and marginal ontologies between the two data sets. Fig. \ref{fig:venn} illustrates this module level comparison. When using exact equality to define the intersection between the two sets of ontologies only find two ontologies were identified by both single-cell and population pipelines. However, many of the output modules share common ancestrial ontologies. To get a better understanding of the similarities of the pipelines' output, we report what we will be calling 'consolidated ontologies' in the intersection. Consolidated ontologies are ancestral ontologies from the Gene Ontology Consortium’s ontology hierarchy database that link at least one ontology reported by the single-cell workflow to at least one ontology reported by the population workflow that is no more than two generations ancestor to the output ontologies from which the consolodated ontology was inferred. If two consolodated ontologies were inferred from the same set of output ontologies, only the most specific consolidated ontology is reported.

\begin{figure}[h]
\includegraphics[width=120mm]{{Figures/toDavidGSE57872_freqNet_03_clusters}}
\caption{\textbf{Single-cell network module ontology enrichment.} A list of the modules output from the CODENSE algorithm ordered by size, each enriched for BP, CC, and MC ontologies. Row color denotes a certain statistical significance as demarcated in the figure legend.}
...
\caption{\textbf{Gene-level starry map comparison of output modules.} }
